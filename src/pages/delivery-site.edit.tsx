...
export const DeliverySiteEditPage = () => {
  const { safeNavigate } = useUnsavedChangesContext();
  const [searchParams] = useSearchParams();
  const dsId = searchParams.id;

  // Convert dsId to number if it exists
  const deliverySiteId = createMemo(() => (dsId ? parseInt(dsId as string, 10) : undefined));
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        breadcrumb={{
          home: i18n.t("requestsManager.home"),
          current: !!deliverySiteId()
            ? i18n.t("deliverySite.editDeliverySite")
            : i18n.t("deliverySite.createNewDeliverySite"),
        }}
        actionButton={{
          label: i18n.t("deliverySite.viewAllDeliverySites"),
          icon: (
            <svg
              aria-hidden="true"
      ...
      <div class="border-t border-gray-200 bg-gray-50/70">
        <div class="divide-y divide-gray-200 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
          <EditDeliverySite deliverySiteId={deliverySiteId()} />
        </div>
      </div>
      {/*  END Right Column Contents */}
    </div>
  );
};
